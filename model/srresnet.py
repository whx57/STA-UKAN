...
class _NetG(nn.Module):
    def __init__(self):
        ...
        self.conv_input = nn.Conv2d(in_channels=256, out_channels=64, kernel_size=9, stride=1, padding=4, bias=False)
        self.relu = nn.LeakyReLU(0.2, inplace=True)
        ...
        self.upscale6x = nn.Sequential(
            ...
            nn.Conv2d(in_channels=64, out_channels=576, kernel_size=3, stride=1, padding=1, bias=False),
            nn.PixelShuffle(3),
            nn.LeakyReLU(0.2, inplace=True),
        )
...
if __name__ == "__main__":
    ...
    # 生成输入数据
    input_tensor = torch.randn(1, 256, 32, 48)
    ...
    print(output.shape)
    '''
    https://github.com/twtygqyy/pytorch-SRResNet/tree/master
    https://arxiv.org/abs/1609.04802)
    batchSize：训练批次大小，默认为 16。
nEpochs：训练的总轮数，默认为 500。
lr：学习率，默认为 1e-4。
step：每step轮对学习率进行衰减，默认为 200。
cuda：是否使用 CUDA 加速，默认为 False。
resume：恢复训练的检查点路径，默认为空。
start-epoch：手动设置的起始轮数，默认为 1。
threads：数据加载器使用的线程数，默认为 0。
pretrained：预训练模型的路径，默认为空。
vgg_loss：是否使用基于 VGG 的内容损失，默认为 False。
gpus：指定使用的 GPU ID，默认为 "0"
